...
**Proses eliminasi sistematis sebelum ketemu akar masalah sesungguhnya.** Fix v1.2.8 (urutan spread di `use-note-editor.ts`) diuji ulang — masih lulus, tidak salah. Skenario ketik-tag-lalu-langsung-tutup-halaman (unmount sebelum auto-save 1500ms selesai) diuji — ternyata sudah aman. Bahkan skenario paling dasar — ketik SATU tag lewat keyboard sungguhan (bukan panggilan fungsi langsung, direndernya component React aslinya lewat `@testing-library/react`'s `render()`, bukan cuma hook-nya via `renderHook()` seperti semua test sebelumnya) — juga lulus. `firestore.rules` diperiksa ulang detail, `normalizeNote`/`updateNote` di `notes.service.ts` dibaca ulang baris per baris, semuanya bersih. Sempat mencoba menjalankan Firestore Emulator Suite sungguhan untuk menyingkirkan kemungkinan mock sendiri yang menyembunyikan bug — gagal karena sandbox tidak bisa mengunduh binary emulator (domain diblokir), jadi kembali ke pembacaan kode manual yang jauh lebih teliti.

**Titik balik: riset ke perilaku Next.js App Router yang sebelumnya tidak pernah dipertimbangkan.** Dicari lewat web search dan dikonfirmasi dari dokumentasi resmi + laporan bug Next.js (github.com/vercel/next.js/issues/49553): navigasi client-side antara dua URL yang resolve ke komponen client YANG SAMA (mis. `/notes/A` ke `/notes/B`, keduanya dilayani `notes/[id]/page.tsx`) **tidak selalu** unmount+remount komponen itu — React bisa saja hanya me-re-render instance yang sama dengan parameter baru. Ini artinya `useNoteEditor(noteId)` hook instance-nya BISA bertahan lintas perpindahan catatan — semua `useRef()`-nya (`pendingInputRef`, `inFlightSaveRef`, `autoSaveTimer`) TIDAK ikut ter-reset hanya karena efek subscription-nya (yang punya `[noteId]` di dependency array) jalan ulang.

**Root cause: `mutationFn` di `saveMutation` (`useMutation`) membaca `noteId` langsung dari parameter fungsi `useNoteEditor(noteId)` — bukan dari ref, dibaca ulang setiap render.** Dibuktikan langsung lewat instrumentasi `console.log` sementara di kode (dihapus lagi setelah terbukti): user ketik tag di Catatan A → auto-save 1500ms dijadwalkan → user pindah ke Catatan B sebelum timer menembak (App Router: TIDAK unmount) → `NotePage`/`useNoteEditor` re-render dengan `noteId='B'`, `mutationFn` didefinisikan ulang menutup `noteId='B'` → timer LAMA dari Catatan A akhirnya menembak, memanggil `saveMutation.mutate()` (stable reference dari TanStack Query) → TAPI `mutationFn` yang benar-benar dieksekusi adalah versi TERBARU (menutup `noteId='B'`) → log terbukti: `mutationFn CALLED with noteId closure = note-2` untuk penyimpanan yang dijadwalkan saat mengedit note-1. **Tag yang diketik di Catatan A terkirim ke Catatan B** — bukan sekadar hilang, tapi korupsi data (bisa menimpa tag catatan lain yang sama sekali tidak diminta).

**Fix: `noteId` tidak lagi pernah dibaca dari closure `mutationFn` sama sekali — dikunci eksplisit sebagai bagian dari payload `mutate()` di titik SETIAP batch dijadwalkan/dieksekusi, bukan di titik timer menembak.** `mutationFn` sekarang men-destructure `targetNoteId` dari payload dan memakainya untuk `updateNote(targetNoteId, ...)`. `scheduleAutoSave` mengunci `targetNoteId = noteIdRef.current` PERSIS saat `setTimeout` dijadwalkan (bukan saat menembak). `noteIdRef` disinkronkan lewat `useEffect([noteId])` — pola valueRef yang sama seperti fix `TagInput` v1.2.7. **Ditemukan dan diperbaiki SEKALIGUS di 7 titik panggilan lain** yang sebelumnya juga rentan bug yang sama persis: `handleManualSave`, `handleTogglePin`, `handleTimeCapsuleChange`, `handleSecretChange`, `handleScheduledChange`, dan cleanup unmount/flush-on-note-change — semuanya sekarang menyertakan `targetNoteId` eksplisit. Cleanup effect flush (sebelumnya cuma jalan sekali saat true-unmount, dependency `[]`) diubah menjadi juga jalan setiap `noteId` berubah (dependency `[noteId]`), sehingga pending changes langsung ter-flush SAAT pindah catatan — dan sengaja membaca `noteId` (closure parameter biasa) bukan `noteIdRef.current` di titik ini, karena urutan eksekusi RELATIF antara dua `useEffect` terpisah bukan bagian dari kontrak API publik React yang dijamin (dikonfirmasi lewat riset), sementara closure `noteId` di dalam efek yang sama SELALU menutup render yang benar — pilihan desain yang secara sengaja tidak bergantung pada perilaku yang "teramati tapi tidak dijamin".

**Diverifikasi lewat 4 test di `tests/unit/hooks/use-note-editor-tags.test.tsx`:** reproduksi utama (gagal sebelum fix — nol panggilan `updateNote` untuk catatan asal sama sekali, terbukti via `rerender()` yang mengganti `noteId` TANPA `unmount()` terlebih dahulu, berbeda dari semua test sebelumnya yang selalu pakai satu `noteId` tetap atau unmount penuh), reproduksi unmount murni (masih lulus, tidak regresi), dan regression guard eksplisit untuk field NON-tags (`handleTogglePin`) yang membuktikan fix yang sama juga melindungi jalur panggilan sinkron, bukan cuma jalur `scheduleAutoSave`.

**Bug kedua ditemukan dan diperbaiki: tombol Backspace untuk hapus tag terakhir masih baca `value` prop langsung, bukan `valueRef.current`.** `addTag()`/`removeTag()` sudah diperbaiki dari stale-closure ini di v1.2.7, tapi cabang `Backspace` di `handleKeyDown` (`tag-input.tsx`) terlewat sepenuhnya dari fix itu — kelas bug yang identik, komponen yang sama, cuma jalur kode berbeda. Dibuktikan lewat test yang gagal sebelum fix (bahkan lebih parah dari dugaan awal: tag yang baru saja ditambahkan lewat `addTag` hilang total tanpa jejak, bukan cuma tag yang salah terhapus) — sekarang lulus.

Full suite: 302/302 test lulus (naik dari 299 — net +3: 2 test Backspace, dan test note-switching menggantikan 1 test lama di file yang sama menjadi 4), `tsc`/`eslint --max-warnings 0`/`next build` seluruhnya bersih. Digabungkan kembali dengan seluruh fix v1.2.8 (Canvas/Graph/Settings/Akun) di bawah — keduanya adalah sesi kerja terpisah dengan penemuan berbeda, dicatat sebagai entri changelog terpisah meski dikirim dalam satu ZIP.

Files: `src/hooks/use-note-editor.ts`, `src/components/tags/tag-input.tsx`, `tests/unit/hooks/use-note-editor-tags.test.tsx`, `tests/unit/components/tag-input.test.tsx`, `tests/unit/components/tag-input-real-dom.test.tsx` (baru), `package.json`, `README.md`
...
**Bug tag — root cause KETIGA, murni di layer integrasi yang belum pernah diuji sebelumnya.** v1.2.3 (data-layer, `getDoc()` partial snapshot) dan v1.2.5 (stale closure di `TagInput`) sudah diverifikasi ulang lewat test yang ada — keduanya **tetap lulus**, membuktikan kedua fix itu sendiri valid dan tidak regresi. Tapi tidak ada satu pun test sebelumnya yang menguji `useNoteEditor` secara utuh — titik tempat `subscribeToNote`'s callback `onSnapshot` dan `scheduleAutoSave`'s `inFlightSaveRef`/`pendingInputRef` benar-benar berinteraksi. Test integrasi baru ditulis (`tests/unit/hooks/use-note-editor-tags.test.tsx`) yang mensimulasikan urutan penuh: ketik tag pertama → auto-save timer menembak → `updateNote()` terkirim tapi belum settle (network delay nyata) → user ketik tag KEDUA sebelum tag pertama terkonfirmasi (hal yang sangat wajar dilakukan) → snapshot server tiba di jendela waktu itu. **Test ini gagal sebelum fix** — tag kedua yang baru diketik hilang, persis gejala yang dilaporkan Vina ("tag ditulis berulang kali tapi kembali lagi kosong").

**Akar masalahnya: urutan spread di `onSnapshot`'s merge logic terbalik.** `pending = { ...pendingInputRef.current, ...inFlightSaveRef.current }` — `inFlightSaveRef` (batch LAMA yang sudah terkirim, snapshot beku saat timer-nya dulu menembak) ditulis di posisi TERAKHIR, sehingga untuk field yang sama, nilai lama ini menimpa `pendingInputRef` (perubahan LEBIH BARU yang baru masuk antrean, belum sempat terkirim). Ini kebalik dari yang seharusnya: karena `pendingInputRef` selalu direset kosong tiap kali suatu batch pindah ke `inFlightSaveRef` (lihat `scheduleAutoSave`), field apa pun yang muncul di `pendingInputRef` **pasti** perubahan yang terjadi setelah batch yang ada di `inFlightSaveRef` — urutan waktu selalu terjaga. **fix: satu baris, urutan spread dibalik** (`{ ...inFlightSaveRef.current, ...pendingInputRef.current }`) sehingga perubahan terbaru yang menang, bukan yang lama.

**Diverifikasi lewat 3 test:** (1) reproduksi utama — tag kedua tidak lagi hilang saat snapshot tiba di tengah save tag ketiga masih in-flight; (2) reproduksi kedua — kasus tag PERTAMA yang hilang total pada catatan baru; (3) **regression guard eksplisit untuk arah sebaliknya** — kasus yang justru dilindungi fix v1.2.5 (field HANYA ada di `inFlightSaveRef`, snapshot basi datang, nilai in-flight harus tetap menang) dipastikan masih bekerja setelah urutan dibalik, supaya fix ini tidak membuka kembali bug yang sudah pernah ditutup.

**Canvas "cuma pajangan, tak bisa ngapa-ngapain" dan Graph "kotaknya masih sempit" — SATU akar masalah yang sama, murni CSS, bukan bug logic di kedua fitur itu sendiri.** Kode `CanvasBoard`, `canvas.service.ts`, dan `CanvasSticky` (drag, ubah warna, hapus, buka catatan) diperiksa satu per satu — semuanya sudah terhubung dengan benar ke Firestore, tidak ada yang dummy/rusak. Masalah sesungguhnya: `src/app/(protected)/layout.tsx`'s wrapper konten memakai `min-h-[calc(100dvh-3.5rem)]` (minimum, bukan tinggi eksplisit). `CanvasPage` dan `GraphPage` sama-sama merender root `<div className="h-full">` yang mengharapkan mengisi 100% tinggi wrapper itu — tapi menurut spesifikasi CSS resmi (CSS 2.1 §10.5, dikonfirmasi konsisten di semua browser, bukan quirk satu browser), `height: 100%` pada elemen anak **hanya** bisa resolve terhadap `height` eksplisit milik leluhur, tidak pernah terhadap `min-height`, walau tinggi hasil render leluhur itu kebetulan memenuhi batas minimumnya. Hasilnya: `h-full` resolve jadi `auto`, yaitu setinggi konten header saja — persis "kotak sempit" yang terlihat di screenshot Graph, dan area drag/pan/zoom Canvas yang secara fisik nyaris tak ada ruang untuk diinteraksi (tombol "+ Sticky" ada di kode tapi terdorong ke luar area terlihat). Ini juga menjelaskan kenapa fix Graph v1.2.7 (menskalakan ulang posisi node saat resize) tidak terasa berdampak — fix itu sendiri valid untuk masalah yang disasarnya (posisi node terjebak di ukuran transisi awal), tapi kalau `parent.clientHeight` **selalu** kecil (bukan cuma sesaat saat transisi), tidak ada perbedaan ukuran untuk di-scale-kan — akar masalah yang lebih dalam belum tersentuh.

**fix: satu baris di `layout.tsx`** — wrapper diubah dari `min-h-[calc(100dvh-3.5rem)]` menjadi `h-[calc(100dvh-3.5rem)] overflow-y-auto`. Diverifikasi aman untuk halaman lain (Dashboard, Catatan, Pengaturan) yang kontennya bisa lebih panjang dari layar: dicek dulu tidak ada satu pun tempat di codebase yang bergantung pada scroll level window/document/body (satu-satunya pemakaian `document.body` di `export.service.ts` adalah teknik unduh file via anchor sementara, tidak terkait scroll) — jadi scroll behavior berpindah ke wrapper ini sendiri (`overflow-y-auto`) tanpa mengubah pengalaman apa pun untuk halaman-halaman itu. `Header` dan `Sidebar` sama-sama `position: fixed`, sepenuhnya independen dari wrapper ini. **Fix ini otomatis membereskan Canvas dan Graph sekaligus** tanpa perlu menyentuh `canvas-board.tsx` atau `graph-view.tsx` sama sekali — dikonfirmasi lewat pemeriksaan `graph-view.tsx`'s `canvas.width = parent.clientWidth; canvas.height = parent.clientHeight`, yang sekarang membaca tinggi kontainer yang benar. Diverifikasi lewat production build penuh (`next build` berhasil, seluruh route ter-generate) dan pengecekan langsung ke CSS terkompilasi untuk memastikan output-nya `height`, bukan `min-height`.

**Settings — duplikasi menu ditemukan dan diperbaiki, khusus untuk desktop.** `SettingsPage` (tab "Umum") merender ulang tiga kartu navigasi (Tampilan/Keamanan/Data) yang PERSIS sama dengan yang sudah disediakan `SettingsShell` yang membungkusnya di sidebar kiri — sisa desain lama dari sebelum `SettingsShell` dibuat. Dicek dulu: di **mobile**, `SettingsShell`'s side-nav memang tersembunyi (`hidden sm:flex`) dan hanya tab pill kecil (`sm:hidden`) yang tampil di atas — kartu-kartu besar di `SettingsPage` di situ memang satu-satunya cara navigasi yang nyaman, jadi tidak bisa dihapus total. **fix: kartu navigasi di `SettingsPage` diberi `sm:hidden`**, sehingga hanya tampil di breakpoint yang sama dengan tab pill `SettingsShell` — duplikasi hilang di desktop (seperti screenshot yang dilaporkan Vina), pengalaman mobile tidak berubah.

**Settings — "tidak ada pengaturan akun" dikonfirmasi memang benar, bukan cuma tersembunyi.** Diperiksa `auth.service.ts`: tidak ada satu pun fungsi untuk ubah nama tampilan, ubah email, ubah password langsung, atau hapus akun — kapabilitasnya memang belum pernah dibangun di layer service, bukan sekadar tidak ada tombolnya di UI. **Halaman `/settings/account` baru dibuat**, mencakup yang sudah bisa didukung layer service saat ini: info akun (nama, email, tanggal bergabung — dari Firebase `user.metadata`), dan tombol kirim email reset password (memakai `sendResetEmail()` yang sudah ada dari v1.2.7, dengan dialog konfirmasi sebelum terkirim). Kartu profil di halaman Umum yang sebelumnya menampilkan info akun inline sekarang jadi link ke halaman ini, menghindari duplikasi info. **Catatan jujur — ini BUKAN solusi lengkap:** mengubah nama/email/password secara langsung dan menghapus akun butuh Firebase reauthentication flow yang belum ada di layer service sama sekali — pekerjaan terpisah yang lebih besar dari satu sesi ini, sengaja tidak dipaksakan setengah-jadi.

**Ditemukan saat proses ini, dicatat untuk sesi berikutnya, TIDAK diperbaiki sekarang (di luar cakupan laporan Vina kali ini):** `useConfirmDialog` hook's JSDoc mengklaim ada instance `<ConfirmDialog />` global terpasang di root layout — ternyata tidak pernah direalisasikan; setiap pemanggil merender instance-nya sendiri secara lokal. `canvas-board.tsx` memanggil `confirm()` dari hook ini tapi tidak pernah merender `<ConfirmDialog />` sama sekali, artinya `confirm()`'s Promise di sana tidak akan pernah resolve karena tidak ada dialog visual untuk diklik.

Full suite: 299/299 test lulus (naik dari 296 — 3 test baru untuk bug tag), `tsc`/`eslint --max-warnings 0`/`next build` seluruhnya bersih pasca-perubahan.

Files: `src/hooks/use-note-editor.ts`, `src/app/(protected)/layout.tsx`, `src/app/(protected)/settings/page.tsx`, `src/app/(protected)/settings/account/page.tsx` (baru), `src/components/settings/settings-shell.tsx`, `src/constants/routes.ts`, `tests/unit/hooks/use-note-editor-tags.test.tsx` (baru), `package.json`, `README.md`
